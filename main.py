...
from models import TipoHurto, Hurto
...
def actualizar_tipo_hurto(idTipo: int, tipo: TipoHurto):
    conn = get_connection()
    cursor = conn.cursor()
...
def eliminar_tipo_hurto(idTipo: int):
    ...
    cursor.execute(
        "SELECT idTipo FROM tipo_hurto WHERE idTipo = %s",
        (idTipo,)
    )
    ...
    cursor.execute(
        "SELECT id FROM hurto WHERE idTipoHurto = %s LIMIT 1",
        (idTipo,)
    )
    ...
    cursor.execute(
        "DELETE FROM tipo_hurto WHERE idTipo = %s",
        (idTipo,)
    )
...
def actualizar_hurto(id: int, hurto: Hurto):
    ...
    cursor.execute(
        "SELECT id FROM hurto WHERE id = %s",
        (id,)
    )
...
def eliminar_hurto(id: int):
    ...
    cursor.execute(
        "DELETE FROM hurto WHERE id = %s RETURNING id",
        (id,)
    )
